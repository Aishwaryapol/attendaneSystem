...
function updateOtherElements() {
  ...
  async function getLabeledFaceDescriptions() {
    if (labels.length === 0) {
      console.warn("No labels available for face recognition.");
      return [];
    }
  ...
  video.addEventListener("play", async () => {
    ...
    setInterval(async () => {
      ...
      detectedFaces = results.filter(result => result.label !== "unknown").map(result => result.label);
...
function sendAttendanceDataToServer() {
  ...
  fetch("handle_attendance", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(attendanceData),
  })
    .then(response => response.json())
    .then(data => showMessage(data.message || "Attendance recorded successfully."))
    .catch(error => console.error("Error sending attendance data:", error));
}
